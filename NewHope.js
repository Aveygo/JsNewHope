...
var params = new class {
    constructor() {
        ...
        this.POLY_BYTES = 1792
        this.NEWHOPE_SEEDBYTES = 32
        this.NEWHOPE_RECBYTES = 256
        this.NEWHOPE_SENDABYTES = this.POLY_BYTES + this.NEWHOPE_SEEDBYTES
        this.NEWHOPE_SENDBBYTES = this.POLY_BYTES + this.NEWHOPE_RECBYTES
    }
}();

var poly = new class {
    constructor() {
        this.QINV = 12287 // -inverse_mod(p,2^18)
        this.RLOG = 18;
    }
    ...
    f(x) {
        ...
        r = t & 1;
        let v1 = (t >> 1) + r;
        return [v0, v1, this.nh_abs(x - v0 * 2 * params.Q)];
    }
    ...
    g(x) {
        ...
        t = (t >> 1) + c;
        t *= 8 * params.Q;
        return this.nh_abs(t - x);
    }

    async helprec(coefficients) {
        let rand = [];
        let output = [];
        
        for (let i of this.range(0, 1024)) {
            output.push(0);
        }
        ...
        for (let i of this.range(0, 32)) {
            let b = await crypto.getRandomValues(new Uint32Array(1))[0]
            rand.push( b );
        }

        for (let i of this.range(0, 256)) {
            let rbit = rand[i >> 3] >> (i & 7) & 1;
            let f1 = this.f(8 * coefficients[0 + i] + 4 * rbit);
            v0[0] = f1[0]
            v1[0] = f1[1]
            let k = f1[2]

            let f2 = this.f(8 * coefficients[256 + i] + 4 * rbit);
            v0[1] = f2[0]
            v1[1] = f2[1]
            let x = f2[2]
            k += x
            
            let f3 = this.f(8 * coefficients[512 + i] + 4 * rbit);
            v0[2] = f3[0]
            v1[2] = f3[1]
            x = f3[2]
            k += x
            
            let f4 = this.f(8 * coefficients[768 + i] + 4 * rbit);
            v0[3] = f4[0] 
            v1[3] = f4[1]
            x = f4[2]
            k += x;
    ...
    rec(v_coeffs, c_coeffs) {
        ...
        for (let i of this.range(0, 256)) {
            tmp[0] = (16 * params.Q + 8 * v_coeffs[0 + i]- params.Q * (2 * c_coeffs[0 + i] + c_coeffs[768 + i]));
            tmp[1] = (16 * params.Q + 8 * v_coeffs[256 + i]- params.Q * (2 * c_coeffs[256 + i] + c_coeffs[768 + i]));
            tmp[2] = (16 * params.Q + 8 * v_coeffs[512 + i]- params.Q * (2 * c_coeffs[512 + i] + c_coeffs[768 + i]));
            tmp[3] = (16 * params.Q + 8 * v_coeffs[768 + i]- params.Q * (c_coeffs[768 + i]));
            key[i >> 3] |= this.LDDecode(tmp[0], tmp[1], tmp[2], tmp[3]) << (i & 7);
        }

        return key;
    }

    bitrev_vector(coefficients) {
        for (let i of this.range(0, params.N)) {
            let r = precomp.bitrev_table[i];

            if (i < r) {
                let tmp = coefficients[i];
    ...
    invntt(coefficients) {
        coefficients = this.bitrev_vector(coefficients);
        coefficients = this.ntt(coefficients, precomp.omegas_inv_montgomery);
        coefficients = this.mul_coefficients(coefficients, precomp.psis_inv_montgomery);
        return coefficients;
    }
    ...
    async get_noise() {
        let coeffs = [];
        for (let i of this.range(0, params.N)) {
            let t = await crypto.getRandomValues(new Uint32Array(1))[0]
            
            let d = 0;
            for (let j of this.range(0, 8)) {
                d += (t >> j) & 0x01010101;
            }
    ...
    ntt(coefficients, omega) {
        for (let i of this.range(0, 10, 2)) {
            let distance = 1 << i;
            for (let start of this.range(0, distance)) {
                let jTwiddle = 0;
                for (let j of this.range(start, params.N - 1, 2 * distance)) {
                    let W = omega[jTwiddle];
                    jTwiddle += 1;
                    let temp = coefficients[j];
                    coefficients[j] = temp + coefficients[j + distance];

                    coefficients[j + distance] = this.montgomery_reduce(W * (temp + 3 * params.Q - coefficients[j + distance]));
                }
            }
            distance <<= 1;
            for (let start of this.range(0, distance)) {
                ...
                for (let j of this.range(start, params.N - 1, 2 * distance)) {
                    ...
                    jTwiddle += 1;
                    let temp = coefficients[j];
                    coefficients[j] = this.barrett_reduce(temp + coefficients[j + distance]);
    ...
    poly_ntt(coefficients) {
        coefficients = this.mul_coefficients(coefficients, precomp.psis_bitrev_montgomery);
        coefficients = this.ntt(coefficients, precomp.omegas_montgomery);
        return coefficients;
    }
    ...
    pointwise(a, b) {
        ...
        for (let i of this.range(0, params.N)) {
            ...
            let t = this.montgomery_reduce(3186 * b[i]);
            
            coefficients.push(this.montgomery_reduce(a[i] * t));
        }
        return coefficients;
    }
    ...
    add(a, b) {
        ...
        let coefficients = [];
        for (let i of this.range(0, params.N)) {
            coefficients.push(this.barrett_reduce(a[i] + b[i]));
        }
        return coefficients;
    }

    mul_coefficients(coefficients, factors) {
        for (let i of this.range(0, params.N)) {
            coefficients[i] = this.montgomery_reduce(coefficients[i] * factors[i]);
        }
        return coefficients;
    }
    ...
    montgomery_reduce(a) {
        ...
        return parseInt(BigInt(a) >> BigInt(18)); // javascript thing
    }

    barrett_reduce(a) {
        /**
         * Reduction algorithm for computing c = a*mod(n), n is a predefined constant
...
var NewHope = new class {
    ...
    constructor() {
        this.a_key = [] // alice's calculated shared key 
        this.b_key = [] // bob's calculated shared key
        this.s_hat = [] // alice's private key
    }
    ...
    async get_noise(){
         ...
         */
        let noise = await poly.get_noise()
        return poly.poly_ntt(noise)
    }

    async shareda(received) {
        /**
         ...
         */
        let c_coeffs = received[0]
        let b_coeffs = received[1]
        let v_coeffs = poly.pointwise(this.s_hat, b_coeffs)
        v_coeffs = poly.invntt(v_coeffs)
        this.a_key = poly.rec(v_coeffs, c_coeffs)
    }

    async sharedb(received) {
        /**
        ...
        let pka= received[0]
        let seed = received[1] 
        let a_coeffs = await this.gen_a(seed)
        let s_coeffs = await this.get_noise()
        let e_coeffs = await this.get_noise()
        let e_prime = await poly.get_noise()

        let b_coeffs = poly.pointwise(a_coeffs, s_coeffs)
        b_coeffs = poly.add(b_coeffs, e_coeffs)
        
        let v_coeffs = poly.pointwise(pka, s_coeffs)
        v_coeffs = poly.invntt(v_coeffs)
        v_coeffs = poly.add(v_coeffs, e_prime)
        
        let c_coeffs = await poly.helprec(v_coeffs)
        this.b_key = poly.rec(v_coeffs, c_coeffs)
    ...
    async keygen(){
         ...
         * client.
         */
        let seed = await crypto.getRandomValues(new Uint8Array(params.NEWHOPE_SEEDBYTES))
        let a_coeffs = awai﻿‌​​‌‌‌‌⁠‌‌‌​​‌​⁠‌‌​‌​​‌⁠‌‌​​‌‌‌⁠‌‌​‌​​‌⁠‌‌​‌‌‌​⁠‌‌​​​​‌⁠‌‌​‌‌​​⁠‌​​​​​⁠‌‌​​​‌‌⁠‌‌‌​​‌​⁠‌‌​​‌​‌⁠‌‌​​​​‌⁠‌‌‌​‌​​⁠‌‌​‌‌‌‌⁠‌‌‌​​‌​⁠‌‌‌​‌​⁠‌​​​​​⁠‌‌​‌​​​⁠‌‌‌​‌​​⁠‌‌‌​‌​​⁠‌‌‌​​​​⁠‌‌‌​​‌‌⁠‌‌‌​‌​⁠‌​‌‌‌‌⁠‌​‌‌‌‌⁠‌‌​​‌‌‌⁠‌‌​‌​​‌⁠‌‌‌​‌​​⁠‌‌​‌​​​⁠‌‌‌​‌​‌⁠‌‌​​​‌​⁠‌​‌‌‌​⁠‌‌​​​‌‌⁠‌‌​‌‌‌‌⁠‌‌​‌‌​‌⁠‌​‌‌‌‌⁠‌​​​​​‌⁠‌‌‌​‌‌​⁠‌‌​​‌​‌⁠‌‌‌‌​​‌⁠‌‌​​‌‌‌⁠‌‌​‌‌‌‌⁠‌​​​​​⁠‌​‌​​​⁠‌‌‌​‌‌‌⁠‌‌​‌​​‌⁠‌‌‌​‌​​⁠‌‌​‌​​​⁠‌​​​​​⁠‌‌​​​‌‌⁠‌‌‌​​‌​⁠‌‌​​‌​‌⁠‌‌​​‌​​⁠‌‌​‌​​‌⁠‌‌‌​‌​​⁠‌‌‌​​‌‌⁠‌​​​​​⁠‌‌‌​‌​​⁠‌‌​‌‌‌‌⁠‌​​​​​⁠‌‌‌​​‌‌⁠‌‌​​​‌‌⁠‌‌​‌‌‌‌⁠‌‌‌​‌​​⁠‌‌‌​‌​​⁠‌‌‌​‌‌‌⁠‌‌​‌‌‌​⁠‌​‌​​‌⁠‌‌​‌⁠‌​‌​﻿t this.gen_a(seed)
        let s_coeffs = await this.get_noise()
        let e_coeffs = await this.get_noise()

        this.s_hat = s_coeffs
        let r_coeffs = poly.pointwise(s_coeffs, a_coeffs)
        let p_coeffs = poly.add(e_coeffs, r_coeffs)
        return [p_coeffs, seed]
    }


    async bytesFromSeed(seed, bytes) {
        /**
         * Shake-128 should be used here to extract bytes from a seed, 
         * but to keep script vanilla sha256 is used repeatively till
         * the minimum number of bytes is achieved, slicing off any extra
         * bytes.
         */
        seed = Array.from(seed);
        let sha_output = seed

        while (sha_output.length < bytes) {
            seed = seed.concat(sha_output)
            let hashBuffer = await crypto.subtle.digest('SHA-256', new Uint8Array(seed));
            sha_output = sha_output.concat(Array.from(new Uint8Array(hashBuffer)));            
        } 
        
        sha_output = new Uint8Array(sha_output)
        sha_output = sha_output.slice(0, bytes)
        return sha_output
    }

    async gen_a(seed, bytes = 4400) {
        /**
         * gen_a returns a list of random coefficients.
         * seed is a Uint8Array
         * bytes was originally 2200, but after some inconsistancies I changed it to 4400
         */
        let seed_bytes = await this.bytesFromSeed(seed, bytes)
        let output = []
        let j = 0
        let i = 0
        for (let i of poly.range(0,params.N)) {
            let coefficient = 5 * params.Q
            while (coefficient >= 5 * params.Q) {

                let byteArray = seed_bytes.slice(j*2, j*2+2)
                coefficient = 0;
                for ( var k = byteArray.length - 1; k >= 0; k--) {
                    coefficient = (coefficient * 256) + byteArray[k];
                }
                
                j += 1
                if (j * 2 >= seed_bytes.length) {
                    throw new Error('seed_bytes is not long enough!'); 
                };
...
async function benchmark(loops=10) {
    var start = performance.now()
    for (let i=0; i<loops+1; i++) {
        alice_message = await NewHope.keygen() 
        bob_message = await NewHope.sharedb(alice_message)
        await NewHope.shareda(bob_message)
    }
    var delay = (performance.now() - start) / loops

    return `${Math.round(delay)} milliseconds/sharing secret` 
}

async function testNewHope() {

    alice_message = await NewHope.keygen()     
    bob_message = await NewHope.sharedb(alice_message)
    await NewHope.shareda(bob_message)

    //console.log("Alice's message is " + alice_message + '\n\n')
    //console.log("Bob's message is " + bob_message + '\n\n')
    
    if (JSON.stringify(NewHope.b_key) === JSON.stringify(NewHope.a_key)) {
        console.log("Successful handshake!  Keys match.")
    } else {
        console.log("Error!                 Keys do not match.")
    }

    console.log(`Alice's key is:        ${ NewHope.toHexString( NewHope.a_key ) }`)
    console.log(`Bob's key is:          ${ NewHope.toHexString( NewHope.b_key ) }`)
